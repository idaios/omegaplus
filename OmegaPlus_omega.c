...
void boundariesGridFile(FILE *gridFile,
			long int *curPos,
			int *readUnsuccessful,
			omega_struct** omstruct, 
			int *grid, 
			int* intsnps, 
			int snpsize, 
			int minw, 
			...
			)
{
	...
	int i, left_boundary, right_boundary, left_min_boundary, right_min_boundary,
	    maxsizematrix = 0, dif;
	int checksnp=0;
	float omega_position = (float)startsnp;
	char line[1024];
	i = 0;
	...
	if( *readUnsuccessful == 1 ){
	  fseek(gridFile, *curPos, SEEK_SET);
	}
	    
	*curPos = ftell(gridFile);
	...
	while( fgets(line, sizeof(line), gridFile) != NULL)
	  {
	    ...
	    if( i + 2 >= *grid ){
	      *grid += *grid;
	      *omstruct = realloc(*omstruct, (*grid) * sizeof(omega_struct));
	    }
	    (*omstruct)[i].omegaRealPos = omega_position = atoi(v2);
	    //fprintf(stderr, "%d ompos: %f\n", i, omega_position);
	    left_boundary = omega_position - maxw;
	    right_boundary = omega_position + maxw;
	    left_min_boundary = omega_position - minw;
	    right_min_boundary = omega_position + minw;
	...
	*grid = i + 2;
	...
	omega_position = (float)startsnp;
	for(i=0; i <= (*grid); i+=(*grid)-1)
	  {
	    (*omstruct)[i].omegaRealPos = omega_position;
	    left_boundary = omega_position - maxw;
	    right_boundary = omega_position + maxw;
...
int findOmegaBoundsGridFile (FILE* gridFile, alignment_struct * alignment, omega_struct ** omega, int *grid, int * maxw, int minw, int minsnps)
{
	int changemaxw = 1,
	  matrixSizeMax=0,
	  maxwc = *maxw; //, i=0;
	long int gridFileCurPos = 0;
	int readUnsuccessful = 0;
	
	while (changemaxw)
	{
		changemaxw = 0;
		boundariesGridFile(gridFile,  &gridFileCurPos, &readUnsuccessful, omega, grid, alignment-> positionsInd, alignment->segsites, 	minw, minsnps, maxwc, &changemaxw, &matrixSizeMax);
		//fprintf(stderr, "%d %ld %d\n", changemaxw, gridFileCurPos, readUnsuccessful);
		
		/* for(i = 0; i < *grid; ++i) */
		/*   fprintf(stderr, "%d %f\n", i, (*omega)[i].omegaRealPos); */
...
#endif
#endif	
	//if( omega[omegaIndex].omegaPos >  17960323 && omega[omegaIndex].omegaPos < 17960325)
