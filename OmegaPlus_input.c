...
void printVersion (FILE * fp)
{
	...
	fprintf(fp,"\tReleased:\t\tJanuary 2018\n\n");
	fprintf(fp,"\tComments:\t\t");
	fprintf(fp,"Input gridpoints from a file\n");
	fprintf(fp,"\n\n");
...
void printHelp (FILE * fp)
{
	...
	fprintf(fp, "\t-gridFile <FILENAME>\tAdd gridpoints from a file (for VCF)\n\n");
	fprintf(fp,"\n\n");
}
	...
	if(filterOut==0)
	{
		for(i=0;i<alignment->segsites;i++)
		{
			...
			for(j=0;j<alignment->sequences;j++)
			{
				...
				dif  = setVec[0]!=0?1:0;
				dif += setVec[1]!=0?1:0;
				//fprintf(stderr, "mis: %d\n", setVec[2]);
				if( setVec[2] > max_perc_missing * alignment->sequences)
				  {
				    rm = 1;
...
int getGTdata (char * string, char * stateVector, int statesTotal, char * sampleData)
{
  int i, j=0, jj = 0, index=0, start=0, end=0, len = strlen(string), na = 0;
...
int processSampleVCF (char * string, int GTpos, char ** sampleData, int * sampleDataMemSize, char * stateVector, int statesTotal)
{
  
  int i, dataSize = getGTfield (string,GTpos), na = 0;;

  if(ploidy > 0)
    dataSize = getGTfield (string,GTpos);
  ...
  if(dataSize+1>*sampleDataMemSize)
	...
	na = getGTdata (string, stateVector, statesTotal, *sampleData);
...
int readLine_VCF (FILE * fp, char ** string, int lineIndex, alignment_struct * alignment, int * DIM, char ** sampleData, int * sampleDataMemSize, int * SNP_SZ, char ** tmpSNPtable, int * tmpSNPtableLineIndex)
{
  ...
  int eol=0, eof=0, maxLength=STRINGLENGTH, na = 0;;
	...
	if(lineSkipped!=1)
	{
		...
		while(getNextString (fp, string, &eol, &eof, &maxLength)==1)
		{
			...
			if(alignment->VCFsample_valid[sampleIndex]==1)
			{
			  ...
			  na += processSampleVCF(*string, GTpos, sampleData, sampleDataMemSize, stateVector, statesALT);
